clc
clear all
basePath =  '\PIBIC\2022-2023\CODIGOS\BaseVFF\BaseMatlab8bits\';
resolutions(10000,2) = 0;
contador = 1;
saveDir = '\PIBIC\2022-2023\Datasets';
...
load('\PIBIC\2022-2023\CODIGOS\BaseVFF\fraturados.mat')
load('\PIBIC\2022-2023\CODIGOS\BaseVFF\classes-info.mat')
% return
numSlices(64,5) = 0;
        ...
        for j=1:5
            if (fraturados(i,j) == 0 && a(i,1) > 1)
                ...
                for z=middle-2:middle+3
                    ...
                    IOriginal = I(boundingBox(2)-2:boundingBox(2)+boundingBox(4)+2,boundingBox(1)-2:boundingBox(1)+boundingBox(3)+2);
                    ...
                    cropAndSaveImages(ICropped, ROI, saveDir, roiName, classe, IOriginal);
...
function cropAndSaveImages(Image, Mask, saveDir, roiName, label, Original)
...
for i = 1:length(percentages)
    ...
    if (isequaln(subset, "90") || isequaln(subset, "60") || isequaln(subset, "30") || isequaln(subset, "10"))
        ...
        imshow(Original);
        hold on;
        plot(imageBoundary(:, 2), imageBoundary(:, 1), 'r', "LineWidth", 6);
        plot(croppedImageBoundary(:, 2), croppedImageBoundary(:, 1), 'c', "LineWidth", 6);
