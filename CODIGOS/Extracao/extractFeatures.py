...
# File variables
#CASES_ROOT = 'C:/PIBIC/2022-2023/Datasets/95/'
CASES_ROOT = 'C:/PIBIC/2022-2023/Datasets/'
ROOT = 'C:/PIBIC/2022-2023/CODIGOS/Extracao/'
...
#INPUTCSV = os.path.join(CASES_ROOT, 'NewCases.csv')
#OUTPUTCSV = os.path.join(CASES_ROOT, 'FeaturesNewCases.csv')
...
NUM_OF_WORKERS = int(cpu_count() / 2)
...
def run(case):
  ...
  try:
    ...
    if os.path.isfile(output_filename):
      ...
      # Extract features
      feature_vector.update(extractor.execute(imageFilepath, maskFilepath))
    ...
    if "imageFilePath" in locals():
      ptLogger.error(f"Image File Path: {imageFilepath}")
...
if __name__ == '__main__':
  ...
  for i in range(5, 105, 5):
    INPUTCSV = os.path.join(CASES_ROOT, f'{i:02}', 'NewCases.csv')
    OUTPUTCSV = os.path.join(CASES_ROOT, f'{i:02}', 'FeaturesNewCases.csv')
    logger.info(f'Loading CSV {i:02}...')
    ...
    if not os.path.isdir(TEMP_DIR):
      logger.info('Creating temporary output directory %s', TEMP_DIR)
      os.mkdir(TEMP_DIR)
